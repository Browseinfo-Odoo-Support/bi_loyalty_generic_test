...
class ReedemptionLoyalty(models.TransientModel):
	...
	def default_get(self, fields):  
		...
		if result:	
			...
			for loop in config:
				if loop.loyalty_tier.min_range <= result.total_sales <= loop.loyalty_tier.max_range:
					for rule in loop.redeem_ids:
						if rule.min_amt <= partner_id.loyalty_points and partner_id.loyalty_points <= rule.max_amt :
							loyalty_balance = round(partner_id.loyalty_points/ rule.points_redeem,2)
	...
	def ok_redeem(self):
		...
		for config in loyalty_config:
			if config:	
				if config.loyalty_tier.min_range <= result.total_sales <= config.loyalty_tier.max_range:
					point_amt = 0
					for rule in config.redeem_ids:
						if rule.min_amt <= partner_id.loyalty_points  and   partner_id.loyalty_points <= rule.max_amt :
							redeem_value = rule.reward_amt
							point_amt+=(int(self.redeem_point) * redeem_value)/rule.points_redeem
