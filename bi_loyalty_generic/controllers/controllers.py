...
class WebsiteSaleInherit(WebsiteSale):
	...
	def get_loyalty_points(self , **post):
		...
		if so:
			...
			for loop in config:
				if loop.loyalty_tier.min_range <= so.total_sales <= loop.loyalty_tier.max_range:
					for rule in loop.redeem_ids:
						if rule.min_amt <= partner.loyalty_points and partner.loyalty_points <= rule.max_amt :
							loyalty_amount_balance = round(partner.loyalty_points/ rule.points_redeem,2)
			data ={
				'partner' : partner.name,
				'points' : partner.loyalty_points,
				'loyalty_amount' : loyalty_amount_balance,
		...
		for config in loyalty_config:
			if config:
				if config.loyalty_tier.min_range <= so.total_sales <= config.loyalty_tier.max_range:
					for rule in config.redeem_ids:
						if rule.min_amt <= so.partner_id.loyalty_points  and so.partner_id.loyalty_points <= rule.max_amt :
							
							res = request.env['sale.order.line'].sudo().create({
								'product_id': config.product_id.id,
								'name': config.product_id.name,
								'price_unit': -(int(post.get('entered_points')) * float(post.get('redeem_value')))/rule.points_redeem,
								'order_id': so.id,
							...
							so.sudo().write({
								...
								'order_redeem_points' :  (int(post.get('entered_points'))*rule.reward_amt)/rule.points_redeem ,
							})
...
class PortalLoyaltyHistory(CustomerPortal):
	...
	def portal_my_history(self,report_type=None, category='', search='',access_token=None, download=False, **kwargs):
		...
		return http.request.render('bi_loyalty_generic.portal_loyalty_history',{
